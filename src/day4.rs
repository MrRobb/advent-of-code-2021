    ...
    fn is_winner(&self) -> bool {
        ...
        let row_winner = self.numbers
            ...
            let col_winner = (0..WIDTH)
                .flat_map(|col| {
                    self.numbers.iter().skip(col).step_by(WIDTH)
                })
                .chunks(WIDTH)
                .into_iter()
                .any(|chunk| chunk
...
pub fn calculate_first_winner(input: &str) -> u32 {
    ...
    let numbers = lines.next().unwrap().to_string().split(',').map(|s| s.parse().unwrap()).collect::<Vec<u32>>();
...
pub fn main() {
    let input = read_to_string("input/day4/input.txt")
